...
def create_invindex(file_csv):
    ...
    with open('terms_index.json', 'w+') as f:
        json.dump(terms_index, f)

    with open('bm_avg.json', 'w+') as f:
        json.dump(bm_avg, f)
...
def upload_collection(collection, connection):
    ...
    for i in range(len(df)):
        ...
        collec_nolyrics[df.iloc[i]['ID']] = example_nol

    with open('collection_nolyrics.json', 'w+') as f:
        json.dump(collec_nolyrics, f)
